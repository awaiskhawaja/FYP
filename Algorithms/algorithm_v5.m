...
%% Fault conditions
fault_bus = 4; % Bus that fault occurs at
...
% I_load = -conj(p_new_load ./ voltages);
...
while running
    ...
    for i = 1:totalBusses
        ...
        if busses(i) == 'c' || busses(i) == 'b'
            ...
            %Q_temp = abs(V(i)) * (Q_ref(i) + (k(i) * (abs(matpower.pfResults.bus(i, 8)) - abs(V(i)))));
